...
def preprocess_text(text,remove_punctuation=True,remove_stopwords=True,stemming=False, lemmatization=False):
    ...
    return ' '.join(words)
df['text_1']=df['text1'].apply(preprocess_text)
df['text_2']=df['text2'].apply(preprocess_text)

print(df)
    


# Loading a pre-trained BERT tokenizer and model
...
def get_bert_embedding(text):
    ...
    return embeddings
df['text1_bert_embeddings']=df['text_1'].apply(get_bert_embedding)
df['text2_bert_embeddings']=df['text_2'].apply(get_bert_embedding)
print(df)

#Saved Df into df_data
import pickle
file=open("df_data.txt","wb")
pickle.dump(df,file)
